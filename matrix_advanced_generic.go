...
// Generic implementations for advanced matrix operations (used as fallback)

// bigMatTransposeGeneric returns the transpose of a 3x3 matrix using pure Go implementation
func bigMatTransposeGeneric(m *BigMatrix3x3, prec uint) *BigMatrix3x3 {
	if prec == 0 {
		prec = m.M[0][0].Prec()
	}
...
// bigMatMulMatGeneric multiplies two 3x3 matrices using pure Go implementation
func bigMatMulMatGeneric(m1, m2 *BigMatrix3x3, prec uint) *BigMatrix3x3 {
	if prec == 0 {
		prec = m1.M[0][0].Prec()
	}
...
// bigMatDetGeneric computes the determinant of a 3x3 matrix using pure Go implementation
func bigMatDetGeneric(m *BigMatrix3x3, prec uint) *BigFloat {
	if prec == 0 {
		prec = m.M[0][0].Prec()
	}
...
// bigMatInverseGeneric computes the inverse of a 3x3 matrix using pure Go implementation
func bigMatInverseGeneric(m *BigMatrix3x3, prec uint) (*BigMatrix3x3, error) {
	if prec == 0 {
		prec = m.M[0][0].Prec()
	}

	// Compute determinant
	det := bigMatDetGeneric(m, prec)
